...
import {
  resolveConsumableUseEnergyCost,
  resolveWeaponUseEnergyCost,
} from "./item-energy-costs";
import { getUnlockedPickedupImportState } from "./runtime-module-gates";
import {
  getWeaponSkillMethodEntries,
  normalizeItemId,
...
const STATEMENT_MECHANICS = statementDamageTable as Record<
  string,
  StatementMechanics
>;
...
function applyPlayerAttackDamage(damage: number): void {
  ...
  if (!gameState.inCombat) {
    appendRuntimeDebug("attack skipped", {
      damage,
...
function resolveStatementMechanics(statementType: string): StatementMechanics {
  const rawValue =
    ...
    STATEMENT_MECHANICS.Statement ??
    DEFAULT_STATEMENT_MECHANICS;
...
function isPurchasedWeapon(itemId: string): boolean {
  ...
  const purchased = useInventoryStore
    .getState()
    .purchasedWeaponIds.map((purchasedItemId) =>
      normalizeItemId(purchasedItemId),
    )
  ...
  return getUnlockedPickedupImportState().weaponItemIds.includes(
    normalizedItemId,
  );
}
...
function isPurchasedConsumable(itemId: string): boolean {
  ...
  const purchased = useInventoryStore
    .getState()
    .purchasedConsumableIds.map((purchasedItemId) =>
      normalizeItemId(purchasedItemId),
    )
    .includes(normalizedItemId);
  ...
  return getUnlockedPickedupImportState().consumableItemIds.includes(
    normalizedItemId,
  );
}
...
function isImportedArmor(itemId: string): boolean {
  const normalizedItemId = normalizeItemId(itemId);
  return getUnlockedPickedupImportState().armorItemIds.includes(
    normalizedItemId,
  );
}
...
function resolveEquippedArmorDefense(
  headArmorId: string,
  bodyArmorId: string,
): number {
  const headArmor = Armors[headArmorId];
  const bodyArmor = Armors[bodyArmorId];
  const headDefense = headArmor
    ? resolveArmorDefenseContribution(headArmor)
    : 0;
  const bodyDefense = bodyArmor
    ? resolveArmorDefenseContribution(bodyArmor)
    : 0;

  return Math.max(0, headDefense + bodyDefense);
}

function applyEquippedArmorSlots(
  headArmorId: string,
  bodyArmorId: string,
): void {
  const player = usePlayerStore.getState();
  const previousArmorDefense = normalizeNonNegativeInt(player.maxDef);
  const nextArmorDefense = resolveEquippedArmorDefense(
    headArmorId,
    bodyArmorId,
  );
  const baseDefense = Math.max(
    0,
    normalizeNonNegativeInt(player.def) - previousArmorDefense,
  );
...
function applyWeaponInflictions(weapon: Weapon): number {
  return weapon.inflictions.reduce((totalBonusDamage, infliction) => {
    ...
    const expectedDamage = Math.max(
      0,
      Math.round(basePerSecond * durationSeconds * infliction.chance),
    );
...
function resolveWeaponSkillIndex(
  weapon: Weapon,
  skillName: string,
  fallbackSkillIndex: number,
): number {
  if (skillName.length === 0) {
    return fallbackSkillIndex;
  }

  const skillEntry = getWeaponSkillMethodEntries(weapon).find(
    (entry) => entry.methodName === skillName,
  );
...
function applyWeaponSkill(
  ...
  const skillIndex = resolveWeaponSkillIndex(
    weapon,
    skillName,
    fallbackSkillIndex,
  );
  ...
  if (!skill) {
    appendTerminalLog(
      `${itemId} has no skill named '${skillName || "skill"}'.`,
    );
    return;
  }
  ...
  if (!trySpendPlayerEnergy(energyCost)) {
    appendTerminalLog(
      `Not enough energy to use ${itemId} skill '${skill.name}'.`,
    );
    return;
  }
  ...
  switch (skill.effect.type) {
    ...
    case "heal": {
      usePlayerStore
        .getState()
        .gainHP(Math.max(0, Math.floor(skill.effect.value)));
      break;
    }
...
function applyConsumableEffects(itemId: string, consumable: Consumable): void {
  ...
  for (const effect of consumable.effects) {
    ...
    if (effect.type === "buff") {
      const multiplier = Number.isFinite(effect.multiplier)
        ? effect.multiplier
        : 1;
      if (multiplier <= 1) {
        continue;
      ...
      if (effect.stat === "dmg") {
        const damageBoost = Math.max(
          1,
          Math.floor(player.baseDmg * (multiplier - 1)),
        );
      ...
      if (effect.stat === "def") {
        const defenseBoost = Math.max(
          1,
          Math.floor(Math.max(1, player.def) * (multiplier - 1)),
        );
    ...
    if (effect.type === "debuff") {
      ...
      if (effect.stat === "speed") {
        useEnemyStore
          .getState()
          .takeEnergyCost(Math.max(1, Math.floor(amount / 2)));
        continue;
      }
    }
  }

  appendRuntimeDebug("consumable used", {
    itemId,
  ...
  switch (event.name) {
    case "builtin.goTo": {
      ...
      const locationId = readString(payload, "locationId", "")
        .trim()
        .toLowerCase();
      const questLevel = normalizeQuestLevel(
        useBountyQuestStore.getState().questLevel,
      );
      ...
      if (
        !ALWAYS_UNLOCKED_GO_TO_SET.has(locationId) &&
        !MACHINE_PROBLEM_PLACE_SET.has(locationId)
      ) {
      ...
      if (
        !ALWAYS_UNLOCKED_GO_TO_SET.has(locationId) &&
        !unlockedPlaces.includes(locationId)
      ) {
        const unlockedTextList = [
          ...ALWAYS_UNLOCKED_GO_TO_SCENES,
          ...unlockedPlaces,
        ];
        const unlockedText =
          unlockedTextList.length > 0 ? unlockedTextList.join(", ") : "none";
        appendTerminalLog(
          `'${locationId}' is locked for level ${questLevel}. Unlocked places: ${unlockedText}.`,
        );
        return;
      }
    ...
    case "game.is_enemy": {
      ...
      appendRuntimeDebug("encounter target set", {
        target: state ? "enemy" : "boss",
      });
      return;
    }
    ...
    case "python.statement": {
      ...
      const hasSpentEnergy = trySpendPlayerEnergy(
        statementMechanics.energyCost,
      );
      if (!hasSpentEnergy) {
        appendTerminalLog(
          `Insufficient energy for ${statementType} (required ${statementMechanics.energyCost}).`,
        );
        return;
      }
    ...
    case "shop.weapon.use": {
      ...
      const weaponEnergyCost = resolveWeaponUseEnergyCost(
        itemId,
        weapon.energyCostPerSwing,
      );
      ...
      const basePlayerDamage = Math.max(
        0,
        Math.floor(usePlayerStore.getState().baseDmg),
      );
    ...
    case "shop.weapon.skill": {
      ...
      const skillIndex = Math.max(
        0,
        Math.floor(readNumber(payload, "skillIndex", 0)),
      );
      ...
      if (!isEnemyCombatActive()) {
        appendTerminalLog(
          `'${itemId}' skill can only be used during enemy combat.`,
        );
        return;
      }
    ...
    case "shop.consumable.use": {
      ...
      if (!isEnemyCombatActive()) {
        appendTerminalLog(
          `'${itemId}' can only be consumed during enemy combat.`,
        );
        return;
      }
      ...
      appendRuntimeDebug("consumable energy spent", {
        itemId,
    ...
    case "player.equip": {
      const itemId = normalizeItemId(
        readString(payload, "itemId", readString(payload, "item", "")),
      );
      ...
      if (armor.slotType === "head") {
        ...
        appendRuntimeDebug("player armor equipped", {
          itemId,
          slotType: armor.slotType,
        });
        return;
      }
    ...
    case "player.unequip": {
      ...
      if (
        nextHeadSlot === currentHeadSlot &&
        nextBodySlot === currentBodySlot
      ) {
